...
def fixture_expected(root, derivatives):
    ...
    return taylor.odejet_padded_scan(vf_autonomous, [root.u0], num=derivatives)
...
def test_output_matches_reference(root: Root, jet_factory: Callable, expected: list):
    derivatives = len(expected) - 1
    init, ssm = probdiffeq.ssm_taylor([root.u0], diffuse_derivatives=derivatives)
    constraint = jet_factory(ssm=ssm)
