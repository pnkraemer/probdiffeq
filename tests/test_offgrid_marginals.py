...
def test_offgrid_marginals_filter(vf, u0, t0, t1, p, solver, info_op):
    ...
    u_left, dense_left = solver.offgrid_marginals(
        t=solution[0].t + 1e-4, state=solution[1], state_previous=solution[0]
    )
    u_right, dense_right = solver.offgrid_marginals(
    ...
    u, dense = solver.offgrid_marginals_searchsorted(ts=ts, solution=solution)
