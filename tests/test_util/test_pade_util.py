from probdiffeq.backend import flow, func, linalg, np, random, structs, testing
...
def test_exact(seed, nrows, ncols, pade_legendre, use_triu):
    ...
    # Evaluate a baseline
    algorithm = exp_gram_matrix_fraction()
    eA1, G1 = func.jit(algorithm)(A, B)

    # Run our algorithm
    algorithm = exp_gram_cholesky(pade_legendre=pade_legendre, solve=solve)
