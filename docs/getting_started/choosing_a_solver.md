...
Almost always, use a `solvers.strategy_filter` strategy for `simulate_terminal_values`, 
a `solvers.strategy_smoother` strategy for `solve_adaptive_save_every_step`,
and a `solvers.strategy_fixedpoint` strategy for `solve_adaptive_save_at`.
