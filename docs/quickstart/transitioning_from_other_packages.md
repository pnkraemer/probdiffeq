...
|---------------------------------------|-----------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|
| `ek0.KroneckerEK0()`                  | `solver_dynamic(Filter(IsoTS0()))`                   | Try other solvers or strategies, depending on the problem.                                                                                  |
| `ek0.DiagonalEK0()`                   | `solver_dynamic(Filter(BlockDiagTS0()))`             | See above.                                                                                                                                  |
| `ek0.ReferenceEK0()`                  | `solver_dynamic(Filter(DenseTS0()))`                 | See above.                                                                                                                                  |
| `ek1.ReferenceEK1()`                  | `solver_dynamic(Filter(DenseTS1()))`                 | See above.                                                                                                                                  |
| `ek1.ReferenceEK1ConstantDiffusion()` | `solver_mle(Filter(DenseTS1()))`                     | See above.                                                                                                                                  |
...
* ProbNumDiffEq.jl calibrates output scales via `DynamicDiffusion`, `FixedDiffusion`, `DynamicMVDiffusion`, or `FixedMVDiffusion`. 
 Their equivalents in ProbDiffEq are `solver_dynamic()` or `solver_mle()`. Feed them with any strategies (Filters/Smoothers) and any state-space model implementations. Use a block-diagonal one (e.g. `BlockDiagTS0()`) for multivariate output scales ("`MVDiffusion`"). Try the `solver_calibrationfree()` with a manual (gradient-based?) calibration if the other routines are unsatisfactory.
...
* Solution objects in ProbDiffEq are random processes (posterior distributions). Random variable types replace most vectors and matrices. This statistical description is richer than a point estimate but needs to be calibrated (e.g. by using an `solver_mle()` instead of `solver_calibrationfree()`)
