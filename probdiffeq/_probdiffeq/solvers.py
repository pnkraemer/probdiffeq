...
class ProbabilisticSolver:
    ...
    def interpolate(
        ...
        transition_t0_t = interp_from.prior.transition(
            dt=t - interp_from.t, output_scale=output_scale
        )
        transition_t_t1 = interp_from.prior.transition(
...
class solver_mle(ProbabilisticSolver):
    ...
    def step(self, state, *, dt: float, damp: float):
        ...
        transition = state.prior.transition(dt=dt, output_scale=output_scale)
...
class solver_dynamic(ProbabilisticSolver):
    ...
    def step(self, state: ProbabilisticSolution, *, dt: float, damp: float):
        ...
        transition = state.prior.transition(dt=dt, output_scale=ones)
...
class error_residual_std(ErrorEstimator):
    ...
    def estimate_error_norm(
        ...
        *,
        ...
        output_scale = np.ones_like(prototype)
        transition = previous.prior.transition(dt=dt, output_scale=output_scale)
