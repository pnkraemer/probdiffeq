...
T = TypeVar("T")
F = TypeVar("F", bound=problems.JetOde | problems.JetResidual, contravariant=True)
...
def jetexpand_ode_padded_scan(*, num: int) -> JetExpansionAlg[problems.JetOde]:
...
def jetexpand_ode_unroll(*, num: int) -> JetExpansionAlg[problems.JetOde]:
    """Taylor-expand the solution of an IVP with Taylor-mode differentiation.
...
def jetexpand_ode_via_jvp(*, num: int) -> JetExpansionAlg[problems.JetOde]:
...
def jetexpand_ode_doubling_unroll(
    *, num_doublings: int
) -> JetExpansionAlg[problems.JetOde]:
...
def jetexpand_ode_coefficient_double() -> JetExpansionAlg[problems.JetOde]:
...
def _error_if_vf_not_odefunction_type(expand):
    ...
    def expand_wrapped(vf, inits, /, *, t: float):
        if not isinstance(vf, problems.JetOde):
            msg = f"Expected type {problems.JetOde}, but got {vf} of type {type(vf)}. "
...
def jetexpand_residual(
...
) -> JetExpansionAlg[problems.JetResidual]:
    ...
    def expand(
        residual: problems.JetResidual, inits: Sequence[T], /, *, t: float
    ) -> tuple[list[T], dict]:
        """Evaluate the Taylor series of a differential-algebraic equation system.
