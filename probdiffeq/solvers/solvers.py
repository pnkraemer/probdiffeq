...
def _step_mle(state, /, dt, vector_field, *, strategy, calibration):
    ...
    # Return
    state = _State(strategy=state_strategy, output_scale=output_scale)
...
class _CalibratedSolver(_Solver):
    ...
    def init(self, t, initial_condition) -> _State:
        ...
        return _State(strategy=state_strategy, output_scale=calib_state)
    ...
    def interpolate(self, t, s0: _State, s1: _State) -> _interp.InterpRes[_State]:
    ...
    def _interp_make_state(self, state_strategy, *, reference: _State) -> _State:
        return _State(state_strategy, output_scale=reference.output_scale)
...
class _UncalibratedSolver(_Solver[_State]):
    def init(self, t, initial_condition) -> _State:
        posterior, output_scale = initial_condition
        state_strategy = self.strategy.init(t, posterior)
        return _State(strategy=state_strategy, output_scale=output_scale)
    ...
    def step(self, state: _State, *, vector_field, dt):
        ...
        state = _State(strategy=state_strategy, output_scale=state.output_scale)
        return dt * error, state
    ...
    def interpolate(self, t, s0: _State, s1: _State) -> _interp.InterpRes[_State]:
        ...
        prev = _State(prev_p, output_scale=s0.output_scale)
        sol = _State(sol_p, output_scale=s1.output_scale)
        acc = _State(acc_p, output_scale=s1.output_scale)
        return _interp.InterpRes(accepted=acc, solution=sol, previous=prev)

    def right_corner(self, state_at_t0, state_at_t1) -> _interp.InterpRes:
        acc_p, sol_p, prev_p = self.strategy.case_right_corner(state_at_t1.strategy)

        prev = _State(prev_p, output_scale=state_at_t0.output_scale)
        sol = _State(sol_p, output_scale=state_at_t1.output_scale)
        acc = _State(acc_p, output_scale=state_at_t1.output_scale)
        return _interp.InterpRes(accepted=acc, solution=sol, previous=prev)
