...
def allclose(
    ...
    *,
    ...
    """
    trees_is_allclose = _tree_allclose(
        tree1, tree2, atol=atol, rtol=rtol, strict_shapes=strict_shapes
    )
    return jax.tree_util.tree_all(trees_is_allclose)


def _tree_allclose(tree1, tree2, /, *, atol, rtol, strict_shapes):
...
def _allclose(
    ...
    /,
    *,
    atol: jax.Array | float | None,
    rtol: jax.Array | float | None,
    strict_shapes: bool,
):
    ...
    values_close = jnp.allclose(a, b, atol=atol, rtol=rtol)
    if not strict_shapes:
        return values_close

    shapes_identical = a.shape == b.shape
    return jnp.logical_and(values_close, shapes_identical)
