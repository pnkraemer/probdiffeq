...
class Strategy(abc.ABC, Generic[S, P]):
    ...
    def begin(self, state: S, /, *, t, dt, parameters, vector_field):
        ...
        #  to this end: extrapolated, observed, corrected into the state.
        #  a "cache" as well? Error estimate (together with an `error_estimate()` method)?
        #  The output scale can remain on solver-level, because this is where we calibrate it.
