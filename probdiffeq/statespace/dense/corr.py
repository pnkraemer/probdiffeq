...
def statistical_order_zero(
    ...
    slr0 = functools.partial(linearise.slr0, cubature_rule=cubature_rule)

    linearise_fun = functools.partial(
        linearise.ode_constraint_0th_noisy,
        ode_shape=ode_shape,
        ode_order=ode_order,
        linearise_fun=slr0,
    )
...
def statistical_order_one(
    ...
    cubature_rule = cubature_rule_fn(input_shape=ode_shape)
    slr1 = functools.partial(linearise.slr1, cubature_rule=cubature_rule)
    linearise_fun = functools.partial(
        linearise.ode_constraint_1st_noisy,
        ode_shape=ode_shape,
        ode_order=ode_order,
        linearise_fun=slr1,
    )
